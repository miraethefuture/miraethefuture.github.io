...
quakeClient.quakes와 quakeClient.quakeLocation(from: quake.url) 함수가 비동기적으로 일어나지만, 동시에 일어나지 않고 순차적으로 진행됨.  
for-in 반복문에서, 반복될 때마다 비동기 함수가 실행됨. 
    - 목록의 첫번째 지진에 대한 위치 정보를 요청 -> 비동기 함수이기 때문에 정보를 가져올 때까지 중단되고 다른 코드가 실행됨 -> 위치 정보 가져오기가 완료되고 다시 해당 코드가 실행됨 -> 다음 suspension point까지 반복문이 실행됨.  
이런 방식은 동시에 진행된다고 볼 수 없음. 
